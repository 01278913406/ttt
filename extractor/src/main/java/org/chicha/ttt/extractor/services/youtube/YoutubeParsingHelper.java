...
import static org.chicha.ttt.extractor.NewPipe.getDownloader;
import static org.chicha.ttt.extractor.utils.Utils.HTTP;
import static org.chicha.ttt.extractor.utils.Utils.HTTPS;
import static org.chicha.ttt.extractor.utils.Utils.UTF_8;
import static org.chicha.ttt.extractor.utils.Utils.getStringResultFromRegexArray;
...
import org.chicha.ttt.extractor.utils.RandomStringFromAlphabetGenerator;
...
public final class YoutubeParsingHelper {
    ...
    public static boolean areHardcodedClientVersionAndKeyValid()
        ...
        final Response response = getDownloader().post(YOUTUBEI_V1_URL + "guide?key="
                        + HARDCODED_KEY1+HARDCODED_KEY2+HARDCODED_KEY3 + DISABLE_PRETTY_PRINT_PARAMETER, headers, body);
    ...
    private static void extractClientVersionAndKeyFromSwJs()
        ...
        try {
            ...
            danhlua = getStringResultFromRegexArray(response, INNERTUBE_API_KEY_REGEXES, 1);
    ...
    private static void extractClientVersionAndKeyFromHtmlSearchResultsPage()
        ...
        try {
            danhlua = getStringResultFromRegexArray(html, INNERTUBE_API_KEY_REGEXES, 1);
        } catch (final Parser.RegexException ignored) {
        }

        if (isNullOrEmpty(danhlua)) {
    ...
    public static String getKey() throws IOException, ExtractionException {
        ...
        if (areHardcodedClientVersionAndKeyValid()) {
            danhlua = HARDCODED_KEY1+HARDCODED_KEY2+HARDCODED_KEY3;
            return danhlua;
        }
        ...
        // case
        return ANDROID_YOUTUBE_KEY1+ANDROID_YOUTUBE_KEY2+ANDROID_YOUTUBE_KEY3;
    }
    ...
    public static JsonObject getJsonAndroidPostResponse(
        ...
        return getMobilePostResponse(endpoint, body, localization,
                getAndroidUserAgent(localization), ANDROID_YOUTUBE_KEY1+ANDROID_YOUTUBE_KEY2+ANDROID_YOUTUBE_KEY3, endPartOfUrlRequest);
    }
    ...
    public static JsonObject getJsonIosPostResponse(
        ...
        return getMobilePostResponse(endpoint, body, localization, getIosUserAgent(localization),
                IOS_YOUTUBE_KEY1+IOS_YOUTUBE_KEY2+IOS_YOUTUBE_KEY3, endPartOfUrlRequest);
    }
